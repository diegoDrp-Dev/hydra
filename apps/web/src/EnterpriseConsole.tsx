...
export default function EnterpriseConsole({ token, onSignOut }: { token: string; onSignOut: () => void }) {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(wsUrl, ["hydra", token]);
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
      <div className="sidebar-brand"><div className="brand-mark"><span>H</span></div><div><strong>HYDRA</strong><small>ENTERPRISE SOC</small></div></div>
      <div className="workspace-chip"><span className="workspace-avatar">W</span><div><small>WORKSPACE</small><strong>{session?.email.split("@")[1] ?? "HYDRA LAB"}</strong></div><span className="chevron">⌄</span></div>
