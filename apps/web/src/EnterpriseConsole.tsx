...
export default function EnterpriseConsole({ token, onSignOut }: { token: string; onSignOut: () => void }) {
  ...
  const connectionLabel: Record<ConnectionState, string> = { live: "Realtime connected", connecting: "Realtime connecting", reconnecting: "Realtime reconnecting", disconnected: "Realtime disconnected" };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace-main">
      ...
      <div className="workspace-content">
        ...
        {view === "overview" ? <>
          <div className="page-heading"><div><p className="eyebrow">LIVE SECURITY POSTURE</p><h2>{greetingFor(new Date(clock).getHours())}, {greetingSubject}.</h2><p>Here is what needs attention across your environment.</p></div><div className={`last-updated ${connection}`}><i /> {connection === "live" ? "Live" : "API polling"} · refreshed {timeAgo(lastRefreshedAt, clock)}</div></div>
          ...
          <section className="overview-grid">
            ...
            <div className="panel"><PanelTitle title="Incident posture" subtitle="Open response workload" action="Live" /><div className="incident-stack">{data.incidents.slice(0,5).map((incident) => <button key={incident.id} onClick={() => { setView("incidents"); setSelected(incident); }}><div><strong>{incident.title}</strong><small>{incident.alerts?.length ?? 0} alerts · {timeAgo(incident.updatedAt, clock)}</small></div><span>{incident.riskScore}</span></button>)}{!data.incidents.length && <EmptyState text="No correlated incidents" />}</div></div>
          </section>
...
function PanelTitle({ title, subtitle, action }: { title: string; subtitle: string; action: ReactNode }) { return <div className="panel-title"><div><h3>{title}</h3><p>{subtitle}</p></div><div className="panel-title-action">{action}</div></div>; }
