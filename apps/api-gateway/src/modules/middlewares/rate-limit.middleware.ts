...
export async function registerRateLimit(app: FastifyInstance) {
  app.addHook("onRequest", async (request, reply) => {
    ...
    try {
      const result = await redisConnection.eval(script, 1, `koryn:ratelimit:${isAuth ? "auth" : "api"}:${identity}`, windowMs) as [number, number];
      ...
      if (isAuth) return reply.status(503).send(errorBody("RATE_LIMIT_UNAVAILABLE", "Authentication is temporarily unavailable"));
